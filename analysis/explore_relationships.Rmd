...
:red_circle: `TODO$:` Hacer mapa de esto 
...
```{r}
my_titulo<- "scd vs. pre (>1250)"
my_xlab <- "trend (tau) of Snow cover duration"
my_ylab <- "trend (tau) of Precipitation"


# Exploratory by elevatin groups 
ggplot(df, aes(x=tau_scd, y=tau_pre)) + 
  geom_point(col='gray') + 
  xlim(-1,1) + ylim(-1,1) + 
  geom_vline(xintercept=0) +
  geom_hline(yintercept=0) + 
  # facet_wrap(~dem50mean_group) + 
  theme_bw() + 
  labs(title= my_titulo, x = my_xlab, y= my_ylab) +
  theme(strip.background = element_rect(fill = "white")) 
...
ggplot(df, aes(x=sen_slope_scd, y=sen_slope_pre)) + 
  geom_point(col='gray') +
  theme_bw() + 
  labs(title= mytitle, x = myxlab, y= myylab) +
  theme(strip.background = element_rect(fill = "white")) + 
