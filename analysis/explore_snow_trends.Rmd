...
myformula <- formula(tau_scd ~ s(dem50mean, fx=FALSE, k=-1, bs="cr"))
...
ggplot(df, aes(x=dem50mean, y=tau_scd)) + geom_point() + 
  geom_smooth(method=gam, formula = tau_scd ~ s(dem50mea))
...
modgam1 <- gam(tau_scd ~ s(dem50mean, fx=FALSE, k=-1, bs= "cr"), data=df)

modgam1 <- gam(sen_slope_scd ~ s(dem50mean, fx=FALSE, k=-1, bs= "cr"), data=df)
...
ggplot(df, aes(x=dem50mean, y=tau_scmd)) + 
  geom_point(col='grey', alpha=.3) + 
  stat_smooth(method=lm, aes(group=dem50mean_group)) + 
  geom_vline(xintercept = seq(1250,3500, by=250)) + 
  mythemeggplot
